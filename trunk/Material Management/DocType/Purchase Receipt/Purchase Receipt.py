...
#-----------------Validation For Fiscal Year------------------------
 ...
 # Update Stock
  ...
  def create_serial_record(self):
    ...
    for d in getlist(self.doclist, 'purchase_receipt_details'):
      if d.serial_no:
        ...
        for a in serial_nos:
          if a:
            exists = sql("select name, status, docstatus from `tabSerial No` where name = '%s'" % (a)) 
            if exists and exists[0][2] == 2:
              msgprint("Serial No: "+ a +" is trashed")
              raise Exception
            if exists and exists[0][1] != 'Not in Use':
              ...
              sql("update `tabSerial No` set status = 'In Store',pr_no = '%s' where name = '%s'" % (cstr(self.doc.name), cstr(a)))
            else:
              ...
              serial.brand = d.brand
              serial.description = d.description
              serial.item_group = item_details and item_details[0]['item_group'] or ''
              serial.warranty_period = item_details and item_details[0]['warranty_period'] or 0
              serial.status = 'In Store'
              serial.pr_no = self.doc.name
              if d.warehouse:
                serial.warehouse = d.warehouse
              serial.save(1)
  ...
  def cancel_serial_no(self):
    sql("update `tabSerial No` set status = 'Not in Use' where pr_no = '%s' " % cstr(self.doc.name))
