...
cur_frm.cscript.onload = function(doc, cdt, cdn) {
  if(user =='Guest'){
    hide_field(['status', 'naming_series', 'order_lost_reason', 'customer', 'rating', 'fax', 'website', 'territory', 'TerritoryHelp', 'address_line1', 'address_line2', 'city', 'state', 'country', 'pincode', 'address', 'lead_owner', 'market_segment', 'industry', 'campaign_name', 'interested_in', 'company', 'fiscal_year', 'contact_by', 'contact_date', 'last_contact_date', 'contact_date_ref', 'to_discuss', 'More Info', 'follow_up', 'Communication History', 'cc_to', 'subject', 'message', 'Attachment Html', 'Create New File', 'lead_attachment_detail', 'Send Email', 'Email', 'sms_message', 'SMS Html', 'lead_sms_detail', 'Send SMS', 'SMS', 'Create Customer', 'Create Enquiry', 'Next Steps', 'transaction_date', 'type', 'source']);
    doc.source = 'Website';
  }
...
// send sms
// ===============================================================
cur_frm.cscript['Send SMS'] = function(doc,cdt,cdn){
  if(doc.__islocal != 1){
    $c_obj(make_doclist(doc.doctype, doc.name),'send_sms','',function(r,rt){});
  }else{
    msgprint("Please save lead first before sending sms")
  }
}
