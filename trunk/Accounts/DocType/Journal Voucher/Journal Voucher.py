  ...
  def create_remarks(self):
    ...
    for d in getlist(self.doclist, 'entries'):
      ...
      if d.against_voucher and d.debit:
        bill_no = sql("select bill_no, bill_date from `tabPayable Voucher` where name=%s", d.against_voucher)
        if bill_no and bill_no[0][0] and bill_no[0][0].lower().strip() not in ['na', 'not applicable', 'none']:
          bill_no = bill_no and bill_no[0]
          r.append('Rs %s against Bill %s dated %s' % (fmt_money(flt(d.debit)), bill_no[0], bill_no[1] and formatdate(bill_no[1].strftime('%Y-%m-%d')) or ''))
    ...
    for d in getlist(self.doclist, 'entries'):
      if d.against_jv:
        if d.against_jv == self.doc.name:
          ...
          raise Exception
        elif not sql("select name from `tabJournal Voucher Detail` where account = '%s' and docstatus = 1 and parent = '%s'" % (d.account, d.parent)):
