    ...
    if self.doc.bill_no and self.doc.bill_no.lower().strip()  not in ['na', 'not applicable', 'none']:
      ...
      if not self.doc.remarks:
        ...
        if self.doc.ded_amount:
          self.doc.remarks = (self.doc.remarks or '') + NEWLINE + ("Grand Total: %s, Tax Deduction Amount: %s" %(self.doc.grand_total, self.doc.ded_amount))
    else:
      if not self.doc.remarks:
...
# ***************************** SUBMIT *****************************
  ...
  def on_submit(self):
    ...
    # make feed
    get_obj('Feed Control','Feed Control').make_feed('submitted', self.doc.doctype, self.doc.name)
...
# ********************************* CANCEL *********************************
  ...
  def on_cancel(self):
    ...
    # make feed
    get_obj('Feed Control','Feed Control').make_feed('cancelled', self.doc.doctype, self.doc.name)
