...
cur_frm.cscript.onload = function(doc,dt,dn) {
  ...
  if(cp.country == 'India') {
    unhide_field(['tds_applicable','tds_category','Get TDS','tax_code','rate','ded_amount','total_tds_on_voucher','tds_amount_on_advance']);
  }
  else {
    hide_field(['tds_applicable','tds_category','Get TDS','tax_code','rate','ded_amount','total_tds_on_voucher','tds_amount_on_advance']);
  }

  var callback2 = function(doc, cdt, cdn){
  ...
  if(doc.supplier) get_server_fields('get_credit_to','','',doc, dt, dn, 0, callback2);
...
cur_frm.cscript['Get Items'] = function(doc, dt, dn) {
  var callback = function(r,rt) { 
    ...
    refresh_many(['supplier_name','supplier_address','credit_to','entries','purchase_receipt_main','purchase_order_main']);
  }
